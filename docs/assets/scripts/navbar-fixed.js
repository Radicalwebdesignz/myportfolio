...
function activateCurrentSection() {
  ...
  var id1; // init the id of the element that will be activated
  ...
  */
  ...
  var firstSection = sections[sections.length-4];  // get last section
  var firstSectionTooSmall = $(firstSection).height() < $(window).height();

  if (firstSectionTooSmall) {
    var firstSectionTopPos = $(firstSection).offset().top;
    // lastSectionTriggerPos is true if 50% of the last section is visible
    var firstSectionTriggerPos = $(window).height() + $(document).scrollTop() - ($(firstSection).height()/4);
    var lastSectionInView1 = firstSectionTriggerPos > firstSectionTopPos;
  }

  if (firstSectionTooSmall && lastSectionInView1) {
    id1 = $(firstSection).attr('id');
  } else {  // else last section is >= 100% of the view check all sections to find the top one
    sections.each(function() {
      var top1 = $(this).offset().top - navHeight; // get the top & bottom position of the section
      ...
      * it is the active section.
      */
      if (pos1 >= top1 && pos1 <= bottom1) {
        id1 = $(this).attr('id');       // store the id of this section
      }
    });
  }
